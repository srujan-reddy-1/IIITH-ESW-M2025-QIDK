...
class GestureRecognizerHelper(
    ...
    // Stores the timestamp we send with each frame
    // so we can compute end-to-end latency correctly.
    private var latestFrameTimestamp = 0L
    ...
    fun setupGestureRecognizer() {
        ...
        try {
            ...
            Log.e(TAG, "MP Task load error: " + e.message)
    ...
    fun recognizeLiveStream(imageProxy: ImageProxy) {
        ...
        latestFrameTimestamp = SystemClock.uptimeMillis()
        ...
        // Send frame timestamp to MediaPipe (needed for live mode)
        recognizeAsync(mpImage, latestFrameTimestamp)
    }
    ...
    // ========== LIVE STREAM CALLBACK RESULT ==========
    private fun returnLivestreamResult(
        result: GestureRecognizerResult, input: MPImage
    ) {
        val now = SystemClock.uptimeMillis()

        // Calculate end-to-end latency correctly
        val latency = now - latestFrameTimestamp
    ...
    fun recognizeImage(image: Bitmap): ResultBundle? {
        if (runningMode != RunningMode.IMAGE) {
            throw IllegalArgumentException(
                "Called recognizeImage while not in IMAGE mode"
            )
        }
    ...
    fun recognizeVideoFile(videoUri: Uri, inferenceIntervalMs: Long): ResultBundle? {
        if (runningMode != RunningMode.VIDEO)
            throw IllegalArgumentException("not in VIDEO mode")
        ...
        for (i in 0..numberOfFrameToRead) {
            ...
            retriever.getFrameAtTime(
                ...
                ?: run {
                    didErrorOccurred = true
                    gestureRecognizerListener?.onError(
                        "Could not retrieve frame at time"
                    )
                }
        }
